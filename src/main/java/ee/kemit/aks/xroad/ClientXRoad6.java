...
public class ClientXRoad6 {
	...
	public Future<ServiceResponse> send(ServiceRequest request, ServiceRequestSerializer serializer,
			ServiceResponseDeserializer deserializer) throws MalformedURLException, SOAPException {

		if (this.securityServerURL != null) {

			SoapClientImplAdvanced client = new SoapClientImplAdvanced();
			this.setKeyStoreContext(client);

			ServiceResponse<String, String> serviceResponse = client.send(request,
	...
	private void setKeyStoreContext(SoapClientImplAdvanced client) {

		if (this.keyStoreFileName != null && this.keyStoreFileName.length() > 0) {

			// validate security server address
			if (this.securityServerURL != null && !this.securityServerURL.toLowerCase().startsWith("https://")) {
