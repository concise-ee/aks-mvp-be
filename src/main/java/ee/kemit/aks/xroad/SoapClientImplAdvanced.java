...
/**
 * This class represents a SOAP client that can be used for sending SOAPMessage and ServiceRequest objects to SOAP
 * endpoints.
 * <p>
 * Copied from
 ...
 * <p>
 * Original repo https://github.com/petkivim/xrd4j Latest repo https://github.com/nordic-institute/xrd4j
 * <p>
 * <p>
 * Modified to support also mTLS
 */
@Log4j2
public class SoapClientImplAdvanced implements SOAPClient {
	...
	/**
	 * Sends the given message to the specified endpoint and blocks until it has returned the response. Null is returned
	 * if the given url is malformed or if sending the message fails. Serialization and deserialization from/to
	 ...
	 * @param serializer   the ServiceRequestSerializer object that serializes the request to SOAPMessage
	 * @param deserializer the ServiceResponseDeserializer object that deserializes SOAPMessage response to ServiceResponse
	 * @return the ServiceResponse object that is the response to the message that was sent.
	 */
	@SuppressWarnings("rawtypes")
	@Override
	public ServiceResponse send(final ServiceRequest request, final String targetURL,
			final ServiceRequestSerializer serializer, final ServiceResponseDeserializer deserializer)
		...
		String producerNamespaceURI =
				request.getProducer().getNamespaceUrl() == null || request.getProducer().getNamespaceUrl().isEmpty() ?
						"*" :
						request.getProducer().getNamespaceUrl();

		ServiceResponse response = deserializer.deserialize(soapResponse,
				producerNamespaceURI,
				request.isProcessingWrappers());
	...
	/**
	 * Calls listClients meta service and retrieves list of all the potential service providers (i.e., members and
	 * subsystems) of an X-Road instance. Returns a list of list of ConsumerMembers that represent X-Road clients.
	 *
	...
	/**
	 * Calls listCentralServices meta service and retrieves list of all central services defined in an X-Road instance.
	 * Returns a list of ProducerMembers that represent X-Road central services.
	 *
	 * @param url URL of X-Road security server
	...
	public List<ProducerMember> listCentralServices(String url) {
		...
		List<ProducerMember> list = new ListCentralServicesResponseDeserializer().deserializeProducerList(
				response.getData());
	...
	/**
	 * Calls listMethods meta service that lists all the services offered by a service provider. Returns a list of
	 * ProducerMember objects wrapped in ServiceResponse object's responseData variable.
	 *
	 * @param request the ServiceRequest object to be sent
	 * @param url     URL that identifies where the message should be sent
	...
	/**
	 * Calls allowedMethods meta service that lists all the services by a service provider that the caller has
	 * permission to invoke. Returns a list of ProducerMember objects wrapped in ServiceResponse object's responseData
	 * variable.
	 *
	 * @param request the ServiceRequest object to be sent
	 * @param url     URL that identifies where the message should be sent
	...
	/**
	 * This is a helper method for meta service calls that don't have a request body. The method sets the service code,
	 * name space and name space prefix, and removes the service code.
	 *
	 * @param request     the ServiceRequest object to be sent
	 * @param url         URL that identifies where the message should be sent
	 ...
	 * @throws SOAPException         if there's a SOAP error
	 * @throws MalformedURLException MalformedURLException if no protocol is specified, or an unknown protocol is found, or url is null
	 */
	@SuppressWarnings("rawtypes")
	private ServiceResponse listServices(final ServiceRequest request, final String url, final String serviceCode)
			throws SOAPException {
	...
	/**
	 * Calls getSecurityServerMetrics monitoring service that returns a data set collected by environmental monitoring
	 * sensors.
	 *
	 * @param request the ServiceRequest object to be sent
	 * @param url     URL that identifies where the message should be sent
	...
	private boolean isHTTPS() {
		return (this.keyStoreFileName != null && this.keyStoreFileName.length() > 0);
	}
	...
	private void loadSecureRequestContext(String targetURL)
		...
		// keystore files need to have been created from a public-private pair with the following command
		// $ openssl pkcs12 -export -in mtp-xtee-dev.crt -inkey mtp-xtee-dev.key -out mtp-xtee-dev.p12
		// needs "winpty" in front in git bash console under windows
